...
interface ProfileImageUploaderProps {
  ...
  onOffsetChange: (x: number, y: number) => void;
  onScaleChange: (scale: number) => void;
  userId: number; // Adicionar userId para a requisição
  authToken: string; // Adicionar authToken para autenticação
  onUploadSuccess: (imageUrl: string) => void; // Callback para quando o upload for bem-sucedido
  onUploadError: (error: string) => void; // Callback para erros no upload
}
...
const ProfileImageUploader: React.FC<ProfileImageUploaderProps> = ({
  ...
  const [startDragX, setStartDragX] = useState(0);
  const [startDragY, setStartDragY] = useState(0);
  const [initialOffsetX, setInitialOffsetX] = useState(imageOffsetX); // Inicializar com os props
  const [initialOffsetY, setInitialOffsetY] = useState(imageOffsetY); // Inicializar com os props
  const [isUploading, setIsUploading] = useState(false); // Novo estado para controlar o loading
  ...
  const uploadImageToServer = async (file: File) => {
    ...
    if (!apiBaseUrl) {
      ...
      setIsUploading(false); // Parar o loading aqui também
      return;
    }
    // --- FIM DO LOG ---
    ...
    try {
      const response = await axios.put(
        `${apiBaseUrl}/client-image/${userId}/avatar`, // Use apiBaseUrl aqui
        formData,
        {
          headers: {
