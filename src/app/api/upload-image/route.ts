...
// Função para criar o cliente do Google Drive
const createGoogleDriveClient = async () => {
  if (!GOOGLE_CLIENT_EMAIL || !GOOGLE_PRIVATE_KEY) {
    throw new Error("As credenciais do Google Drive não estão configuradas.");
  }
...
/**
 * POST: Cria um novo arquivo no Google Drive.
 * @param {NextRequest} req - A requisição do Next.js.
 * @returns {NextResponse} A resposta JSON com a URL da miniatura.
 */
export async function POST(req: NextRequest) {
  try {
    const drive = await createGoogleDriveClient();
...
/**
 * PUT: Atualiza um arquivo existente no Google Drive.
 * @param {NextRequest} req - A requisição do Next.js.
 * @returns {NextResponse} A resposta JSON com a URL da miniatura.
 */
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!file || !fileId) {
      return NextResponse.json(
        { message: "Nenhum arquivo ou ID do arquivo encontrado." },
        { status: 400 }
      );
    }
