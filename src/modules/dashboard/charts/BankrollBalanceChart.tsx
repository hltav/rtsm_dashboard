// "use client";
// import Typography from "@mui/material/Typography";
// import Stack from "@mui/material/Stack";
// import { LineChart } from "@mui/x-charts/LineChart";
// import { useBankrollContext } from "@/components/Providers/BankrollContext";
// import { useBankrollHistory } from "@/modules/bankroll/hook/useBankrollHistory";
// import { useState, useMemo, useEffect, useCallback, memo } from "react";
// import {
//   MenuItem,
//   Select,
//   FormControl,
//   InputLabel,
//   useTheme,
//   Card,
//   CardContent,
// } from "@mui/material";
// import { useFilteredBankrollHistory } from "../hooks/useFilteredBankrollHistory";
// import { GetBankrollHistoryDTO } from "@/modules/bankroll/schema/bankrollHistory.schema";

// const BankrollDataFetcher = memo(
//   ({
//     id,
//     filterMode,
//     onDataLoaded,
//   }: {
//     id: number;
//     filterMode: "all" | "daily";
//     onDataLoaded: (id: number, data: GetBankrollHistoryDTO[]) => void;
//   }) => {
//     const { data } = useBankrollHistory(id);
//     const filtered = useFilteredBankrollHistory(data ?? [], filterMode);

//     useEffect(() => {
//       if (filtered) {
//         onDataLoaded(id, filtered);
//       }
//     }, [id, filtered, onDataLoaded]);

//     return null;
//   }
// );

// BankrollDataFetcher.displayName = "BankrollDataFetcher";

// interface BankrollBalanceChartProps {
//   bankrollId: number | "all" | null;
// }

// export default function BankrollBalanceChart({
//   bankrollId,
// }: BankrollBalanceChartProps) {
//   const { bankrolls, isLoading: isLoadingContext } = useBankrollContext();

//   const [selectedBankId, setSelectedBankId] = useState<number | "all" | null>(
//     bankrollId ?? null
//   );

//   useEffect(() => {
//     setSelectedBankId(bankrollId);
//   }, [bankrollId]);

//   const isAll = selectedBankId === "all";

//   const {
//     data: bankrollHistory,
//     isLoading: isLoadingHistory,
//     error,
//   } = useBankrollHistory(!isAll ? Number(selectedBankId ?? 0) : 0);

//   const [filterMode, setFilterMode] = useState<"all" | "daily">("all");
//   const [resultFilter, setResultFilter] = useState<
//     "all" | "won" | "lost" | "profitloss"
//   >("all");

//   const theme = useTheme();

//   const bankroll = !isAll
//     ? bankrolls?.find((b) => b.id === selectedBankId)
//     : null;

//   const filteredHistory = useFilteredBankrollHistory(
//     !isAll ? bankrollHistory : [],
//     filterMode
//   );

//   const resultFilteredHistory = useMemo(() => {
//     if (isAll) return [];

//     if (!filteredHistory) return [];
//     return filteredHistory.filter((h) => {
//       if (resultFilter === "all") return true;
//       if (resultFilter === "won") return h.type === "BET_WON";
//       if (resultFilter === "lost") return h.type === "BET_LOST";
//       if (resultFilter === "profitloss")
//         return h.type === "BET_WON" || h.type === "BET_LOST";
//       return true;
//     });
//   }, [isAll, filteredHistory, resultFilter]);

//   const chartData = useMemo(() => {
//     if (isAll) {
//       return {
//         dates: [],
//         balances: [],
//         netProfit: 0,
//         profitColor: "#ecec09",
//       };
//     }

//     const source = resultFilteredHistory;

//     if (!source || source.length === 0) {
//       return {
//         dates: [],
//         balances: [],
//         netProfit: 0,
//         profitColor: "#ecec09",
//       };
//     }

//     const dates = source.map((p) => new Date(p.date));

//     const balances = source.map((p) =>
//       parseFloat((p.balanceAfter ?? p.balanceBefore ?? 0).toString())
//     );

//     const first = balances[0];
//     const last = balances[balances.length - 1];
//     const netProfit = last - first;

//     const profitColor =
//       netProfit >= 0
//         ? theme.palette.mode === "light"
//           ? "#17ad1a"
//           : "#17ad1a"
//         : theme.palette.mode === "light"
//         ? "#c62828"
//         : "#c62828";

//     return { dates, balances, netProfit, profitColor };
//   }, [isAll, resultFilteredHistory, theme]);

//   const { dates, balances, netProfit, profitColor } = chartData;

//   const [allBankDataMap, setAllBankDataMap] = useState<
//     Record<number, GetBankrollHistoryDTO[]>
//   >({});

//   const handleBankDataLoaded = useCallback(
//     (id: number, data: GetBankrollHistoryDTO[]) => {
//       setAllBankDataMap((prev) => {
//         if (prev[id] === data) return prev;
//         return { ...prev, [id]: data };
//       });
//     },
//     []
//   );

//   const allBankHistories = useMemo(() => {
//     if (!isAll) return [];

//     return bankrolls.map((b) => {
//       const historyRaw = allBankDataMap[b.id] || [];

//       const resultFilt = historyRaw.filter((h) => {
//         if (resultFilter === "all") return true;
//         if (resultFilter === "won") return h.type === "BET_WON";
//         if (resultFilter === "lost") return h.type === "BET_LOST";
//         if (resultFilter === "profitloss")
//           return h.type === "BET_WON" || h.type === "BET_LOST";
//         return true;
//       });

//       return {
//         id: b.id,
//         name: b.name,
//         history: resultFilt,
//       };
//     });
//   }, [isAll, bankrolls, allBankDataMap, resultFilter]);

//   const colorPalette = ["#17ad1a", "#c62828", "#1976d2", "#9c27b0", "#ecec09"];

//   const yScaleConfig = useMemo(() => {
//     if (isAll || !bankroll) {
//       return {
//         minValue: undefined,
//         maxValue: undefined,
//       };
//     }

//     const initial = Number(bankroll.initialBalance ?? 0);
//     const range = initial * 0.25;

//     return {
//       minValue: initial - range,
//       maxValue: initial + range,
//     };
//   }, [isAll, bankroll]);

//   return (
//     <Card elevation={0}>
//       {/* Renderização condicional dos Fetchers.
//           Isso permite chamar os hooks para cada ID individualmente de forma válida.
//       */}
//       {isAll &&
//         bankrolls.map((b) => (
//           <BankrollDataFetcher
//             key={b.id}
//             id={b.id}
//             filterMode={filterMode}
//             onDataLoaded={handleBankDataLoaded}
//           />
//         ))}

//       <CardContent>
//         <Stack width="100%">
//           <Stack
//             direction="row"
//             justifyContent="space-between"
//             alignItems="center"
//             sx={{ mb: 2 }}
//           >
//             <Stack direction="row" alignItems="center" spacing={1}>
//               <Typography variant="h6" component="span">
//                 {isAll ? "Todas as bancas" : bankroll?.name}
//               </Typography>
//             </Stack>

//             <Stack direction="row" spacing={4}>
//               <FormControl size="small" sx={{ minWidth: 180 }}>
//                 <InputLabel>Atualizações</InputLabel>
//                 <Select
//                   label="Atualizações"
//                   value={filterMode}
//                   onChange={(e) =>
//                     setFilterMode(e.target.value as "all" | "daily")
//                   }
//                 >
//                   <MenuItem value="daily">Última atualização do dia</MenuItem>
//                   <MenuItem value="all">Todas as atualizações</MenuItem>
//                 </Select>
//               </FormControl>
//               <FormControl size="small" sx={{ minWidth: 180 }}>
//                 <InputLabel>Resultados</InputLabel>
//                 <Select
//                   label="Resultados"
//                   value={resultFilter}
//                   onChange={(e) =>
//                     setResultFilter(
//                       e.target.value as "all" | "won" | "lost" | "profitloss"
//                     )
//                   }
//                 >
//                   <MenuItem value="all">Todos</MenuItem>
//                   <MenuItem value="won">Somente Ganhos</MenuItem>
//                   <MenuItem value="lost">Somente Perdas</MenuItem>
//                   <MenuItem value="profitloss">Lucro/Prejuízo</MenuItem>
//                 </Select>
//               </FormControl>
//             </Stack>
//           </Stack>

//           {isLoadingContext || (!isAll && isLoadingHistory) ? (
//             <Typography sx={{ textAlign: "center", py: 4 }}>
//               Carregando gráfico...
//             </Typography>
//           ) : error && !isAll ? (
//             <Typography color="error" sx={{ textAlign: "center", py: 4 }}>
//               Erro ao carregar dados do gráfico.
//             </Typography>
//           ) : (
//             <>
//               {isAll ? (
//                 <LineChart
//                   xAxis={[
//                     {
//                       scaleType: "point",
//                       data:
//                         allBankHistories[0]?.history.map(
//                           (h) => new Date(h.date)
//                         ) ?? [],
//                       valueFormatter: (date) =>
//                         date.toLocaleString("pt-BR", {
//                           day: "2-digit",
//                           month: "2-digit",
//                           hour: "2-digit",
//                           minute: "2-digit",
//                         }),
//                     },
//                   ]}
//                   series={allBankHistories.map((b, i) => ({
//                     label: b.name,
//                     data: b.history.map((h) =>
//                       parseFloat(
//                         (h.balanceAfter ?? h.balanceBefore ?? 0).toString()
//                       )
//                     ),
//                     color: colorPalette[i % colorPalette.length],
//                     showMark: true,
//                   }))}
//                   height={320}
//                   grid={{ horizontal: true }}
//                 />
//               ) : (
//                 <>
//                   <LineChart
//                     xAxis={[
//                       {
//                         scaleType: "point",
//                         data: dates,
//                         valueFormatter: (date) =>
//                           date.toLocaleString("pt-BR", {
//                             day: "2-digit",
//                             month: "2-digit",
//                             hour: "2-digit",
//                             minute: "2-digit",
//                           }),
//                       },
//                     ]}
//                     yAxis={[
//                       {
//                         min: yScaleConfig.minValue,
//                         max: yScaleConfig.maxValue,
//                       },
//                     ]}
//                     series={[
//                       {
//                         label: "Evolução",
//                         data: balances,
//                         color: profitColor,
//                         showMark: true,
//                         valueFormatter: (v) =>
//                           v?.toLocaleString("pt-BR", {
//                             style: "currency",
//                             currency: "BRL",
//                           }) ?? "",
//                       },
//                     ]}
//                     height={320}
//                     grid={{ horizontal: true }}
//                   />

//                   <Typography
//                     variant="body2"
//                     sx={{
//                       alignSelf: "center",
//                       mt: 1,
//                       color: profitColor,
//                       fontWeight: 600,
//                     }}
//                   >
//                     {netProfit >= 0 ? "Lucro líquido: " : "Prejuízo: "}
//                     {netProfit.toLocaleString("pt-BR", {
//                       style: "currency",
//                       currency: "BRL",
//                     })}
//                   </Typography>
//                 </>
//               )}
//             </>
//           )}
//         </Stack>
//       </CardContent>
//     </Card>
//   );
// }

// "use client";
// import Typography from "@mui/material/Typography";
// import Stack from "@mui/material/Stack";
// import { LineChart } from "@mui/x-charts/LineChart";
// import { useBankrollContext } from "@/components/Providers/BankrollContext";
// import { useState, useMemo, useEffect, useCallback } from "react";
// import {
//   MenuItem,
//   Select,
//   FormControl,
//   InputLabel,
//   useTheme,
//   Card,
//   CardContent,
// } from "@mui/material";
// import { useDailySnapshots } from "@/modules/bankroll/hook/snapshots/useDailySnapshots";
// import { useHourlySnapshots } from "@/modules/bankroll/hook/snapshots/useHourlySnapshots";
// import {
//   buildSeries,
//   BankrollSnapshotFetcher,
// } from "./functions/bankrollBalance.functions";
// import { BankrollBalanceChartProps } from "./interface/bankrollBalance.interface";
// import {
//   FilterMode,
//   ResultFilter,
//   SeriesData,
// } from "./types/bankrollBalance.types";

// export default function BankrollBalanceChart({
//   bankrollId,
// }: BankrollBalanceChartProps) {
//   const { bankrolls, isLoading: isLoadingContext } = useBankrollContext();
//   const theme = useTheme();

//   const [selectedBankId, setSelectedBankId] = useState<number | "all" | null>(
//     bankrollId ?? null,
//   );

//   useEffect(() => {
//     setSelectedBankId(bankrollId);
//   }, [bankrollId]);

//   const isAll = selectedBankId === "all";

//   const [filterMode, setFilterMode] = useState<FilterMode>("all");
//   const [resultFilter, setResultFilter] = useState<ResultFilter>("all");

//   const bankroll = !isAll
//     ? bankrolls?.find((b) => b.id === selectedBankId)
//     : null;

//   // ✅ Single bankroll: busca só o que precisa
//   const bankId = !isAll ? Number(selectedBankId ?? 0) : 0;

//   const dailyQ = useDailySnapshots(bankId, !isAll && filterMode === "daily");
//   const hourlyQ = useHourlySnapshots(bankId, !isAll && filterMode === "all");

//   const isLoadingSnapshots = !isAll && (dailyQ.isLoading || hourlyQ.isLoading);
//   const error = !isAll ? (dailyQ.error ?? hourlyQ.error) : null;

//   const singleSeries = useMemo(() => {
//     if (isAll) return { dates: [], balances: [], netProfit: 0 };

//     if (filterMode === "daily") {
//       return buildSeries(dailyQ.data ?? [], resultFilter);
//     }
//     return buildSeries(hourlyQ.data ?? [], resultFilter);
//   }, [isAll, filterMode, dailyQ.data, hourlyQ.data, resultFilter]);

//   const profitColor =
//     singleSeries.netProfit >= 0
//       ? theme.palette.mode === "light"
//         ? "#17ad1a"
//         : "#17ad1a"
//       : theme.palette.mode === "light"
//         ? "#e42a2a"
//         : "#c62828";

//   // ✅ Todas as bancas: guarda a série pronta por id
//   const [allBankSeriesMap, setAllBankSeriesMap] = useState<
//     Record<number, SeriesData>
//   >({});

//   const handleBankSeriesLoaded = useCallback(
//     (id: number, series: SeriesData) => {
//       setAllBankSeriesMap((prev) => {
//         const prevSeries = prev[id];

//         // micro otimização: evita re-render se não mudou referência
//         if (prevSeries === series) return prev;
//         return { ...prev, [id]: series };
//       });
//     },
//     [],
//   );

//   const allBankSeriesList = useMemo(() => {
//     if (!isAll) return [];

//     return bankrolls.map((b) => ({
//       id: b.id,
//       name: b.name,
//       series: allBankSeriesMap[b.id] ?? {
//         dates: [],
//         balances: [],
//         netProfit: 0,
//       },
//     }));
//   }, [isAll, bankrolls, allBankSeriesMap]);

//   const colorPalette = ["#17ad1a", "#e42a2a", "#1976d2", "#9c27b0", "#ecec09"];

//   const yScaleConfig = useMemo(() => {
//     if (isAll || !bankroll) {
//       return {
//         minValue: undefined as number | undefined,
//         maxValue: undefined as number | undefined,
//       };
//     }

//     const initial = Number(bankroll.initialBalance ?? 0);
//     const range = initial * 0.25;

//     return {
//       minValue: initial - range,
//       maxValue: initial + range,
//     };
//   }, [isAll, bankroll]);

//   return (
//     <Card elevation={0}>
//       {isAll &&
//         bankrolls.map((b) => (
//           <BankrollSnapshotFetcher
//             key={b.id}
//             id={b.id}
//             filterMode={filterMode}
//             resultFilter={resultFilter}
//             onDataLoaded={handleBankSeriesLoaded}
//           />
//         ))}

//       <CardContent>
//         <Stack width="100%">
//           <Stack
//             direction="row"
//             justifyContent="space-between"
//             alignItems="center"
//             sx={{ mb: 2 }}
//           >
//             <Typography variant="h6" component="span">
//               {isAll ? "Todas as bancas" : bankroll?.name}
//             </Typography>

//             <Stack direction="row" spacing={4}>
//               <FormControl size="small" sx={{ minWidth: 220 }}>
//                 <InputLabel>Atualizações</InputLabel>
//                 <Select
//                   label="Atualizações"
//                   value={filterMode}
//                   onChange={(e) => setFilterMode(e.target.value as FilterMode)}
//                 >
//                   <MenuItem value="daily">Última atualização do dia</MenuItem>
//                   <MenuItem value="all">
//                     Todas as atualizações (hora a hora)
//                   </MenuItem>
//                 </Select>
//               </FormControl>

//               <FormControl size="small" sx={{ minWidth: 180 }}>
//                 <InputLabel>Resultados</InputLabel>
//                 <Select
//                   label="Resultados"
//                   value={resultFilter}
//                   onChange={(e) =>
//                     setResultFilter(e.target.value as ResultFilter)
//                   }
//                 >
//                   <MenuItem value="all">Todos</MenuItem>
//                   <MenuItem value="won">Somente Ganhos</MenuItem>
//                   <MenuItem value="lost">Somente Perdas</MenuItem>
//                   <MenuItem value="profitloss">Lucro/Prejuízo</MenuItem>
//                 </Select>
//               </FormControl>
//             </Stack>
//           </Stack>

//           {isLoadingContext || isLoadingSnapshots ? (
//             <Typography sx={{ textAlign: "center", py: 4 }}>
//               Carregando gráfico...
//             </Typography>
//           ) : error && !isAll ? (
//             <Typography color="error" sx={{ textAlign: "center", py: 4 }}>
//               Erro ao carregar dados do gráfico.
//             </Typography>
//           ) : (
//             <>
//               {isAll ? (
//                 <LineChart
//                   xAxis={[
//                     {
//                       scaleType: "point",
//                       data: allBankSeriesList[0]?.series.dates ?? [],
//                       valueFormatter: (date) =>
//                         date.toLocaleString("pt-BR", {
//                           day: "2-digit",
//                           month: "2-digit",
//                           ...(filterMode === "all"
//                             ? { hour: "2-digit", minute: "2-digit" }
//                             : {}),
//                         }),
//                     },
//                   ]}
//                   series={allBankSeriesList.map((b, i) => ({
//                     label: b.name,
//                     data: b.series.balances,
//                     color: colorPalette[i % colorPalette.length],
//                     showMark: true,
//                   }))}
//                   height={320}
//                   grid={{ horizontal: true }}
//                 />
//               ) : (
//                 <>
//                   <LineChart
//                     xAxis={[
//                       {
//                         scaleType: "point",
//                         data: singleSeries.dates,
//                         valueFormatter: (date) =>
//                           date.toLocaleString("pt-BR", {
//                             day: "2-digit",
//                             month: "2-digit",
//                             ...(filterMode === "all"
//                               ? { hour: "2-digit", minute: "2-digit" }
//                               : {}),
//                           }),
//                       },
//                     ]}
//                     yAxis={[
//                       {
//                         min: yScaleConfig.minValue,
//                         max: yScaleConfig.maxValue,
//                       },
//                     ]}
//                     series={[
//                       {
//                         label: "Evolução",
//                         data: singleSeries.balances,
//                         color: profitColor,
//                         showMark: true,
//                         valueFormatter: (v) =>
//                           v?.toLocaleString("pt-BR", {
//                             style: "currency",
//                             currency: "BRL",
//                           }) ?? "",
//                       },
//                     ]}
//                     height={320}
//                     grid={{ horizontal: true }}
//                   />

//                   <Typography
//                     variant="body2"
//                     sx={{
//                       alignSelf: "center",
//                       mt: 1,
//                       color: profitColor,
//                       fontWeight: 600,
//                     }}
//                   >
//                     {singleSeries.netProfit >= 0
//                       ? "Lucro líquido: "
//                       : "Prejuízo: "}
//                     {singleSeries.netProfit.toLocaleString("pt-BR", {
//                       style: "currency",
//                       currency: "BRL",
//                     })}
//                   </Typography>
//                 </>
//               )}
//             </>
//           )}
//         </Stack>
//       </CardContent>
//     </Card>
//   );
// }

"use client";

import Typography from "@mui/material/Typography";
import Stack from "@mui/material/Stack";
import { LineChart } from "@mui/x-charts/LineChart";
import { useBankrollContext } from "@/components/Providers/BankrollContext";
import { useState, useMemo, useEffect, useCallback } from "react";
import {
  MenuItem,
  Select,
  FormControl,
  InputLabel,
  Card,
  CardContent,
  TextField,
} from "@mui/material";
import { useDailySnapshots } from "@/modules/bankroll/hook/snapshots/useDailySnapshots";
import { useHourlySnapshots } from "@/modules/bankroll/hook/snapshots/useHourlySnapshots";
import { useMonthlySnapshots } from "@/modules/bankroll/hook/snapshots/useMonthlySnapshots";
import { buildSeries } from "./functions/bankrollBalance.functions";
import { BankrollBalanceChartProps } from "./interface/bankrollBalance.interface";
import { ResultFilter, SeriesData } from "./types/bankrollBalance.types";
import { ptBR } from "date-fns/locale";
import { BankrollSnapshotFetcher } from "./functions/bankrollSnapshotFetcher";
import { LocalizationProvider } from "@mui/x-date-pickers/LocalizationProvider";
import { DatePicker } from "@mui/x-date-pickers/DatePicker";
import { AdapterDateFns } from "@mui/x-date-pickers/AdapterDateFns";
import { Period, getRange } from "./utils/dateRanges";

export default function BankrollBalanceChart({
  bankrollId,
}: BankrollBalanceChartProps) {
  const { bankrolls, isLoading: isLoadingContext } = useBankrollContext();

  const [selectedBankId, setSelectedBankId] = useState<number | "all" | null>(
    bankrollId ?? null,
  );

  useEffect(() => {
    setSelectedBankId(bankrollId);
  }, [bankrollId]);

  const isAll = selectedBankId === "all";

  // ✅ Novo: período + data selecionada
  const [period, setPeriod] = useState<Period>("day");
  const [selectedDate, setSelectedDate] = useState<Date>(new Date());

  const range = useMemo(
    () => getRange(period, selectedDate),
    [period, selectedDate],
  );

  const [resultFilter, setResultFilter] = useState<ResultFilter>("all");

  const bankroll = !isAll
    ? bankrolls?.find((b) => b.id === selectedBankId)
    : null;

  const bankId = !isAll ? Number(selectedBankId ?? 0) : 0;

  // ✅ Queries por período (só 1 ativa)
  const hourlyQ = useHourlySnapshots(bankId, range, !isAll && period === "day");
  const dailyQ = useDailySnapshots(
    bankId,
    range,
    !isAll && (period === "week" || period === "month"),
  );
  const monthlyQ = useMonthlySnapshots(
    bankId,
    { year: selectedDate.getFullYear() },
    !isAll && period === "year",
  );

  const isLoadingSnapshots =
    !isAll && (hourlyQ.isLoading || dailyQ.isLoading || monthlyQ.isLoading);

  // ✅ pega o erro apenas do período ativo
  const error = useMemo(() => {
    if (isAll) return null;
    if (period === "day") return hourlyQ.error ?? null;
    if (period === "week" || period === "month") return dailyQ.error ?? null;
    return monthlyQ.error ?? null;
  }, [isAll, period, hourlyQ.error, dailyQ.error, monthlyQ.error]);

  const singleSeries = useMemo<SeriesData>(() => {
    if (isAll) return { dates: [], balances: [], netProfit: 0 };

    if (period === "day") return buildSeries(hourlyQ.data ?? [], resultFilter);

    if (period === "week" || period === "month") {
      return buildSeries(dailyQ.data ?? [], resultFilter);
    }

    // year -> monthly (12 pontos)
    return buildSeries(monthlyQ.data ?? [], resultFilter);

    // 🔁 fallback se você ainda não tiver monthly:
    // return buildSeries(dailyQ.data ?? [], resultFilter); // mas aí teria que habilitar dailyQ pro ano inteiro
  }, [isAll, period, hourlyQ.data, dailyQ.data, monthlyQ.data, resultFilter]);

  const profitColor = singleSeries.netProfit >= 0 ? "#17ad1a" : "#c62828";

  // ✅ Todas as bancas (igual ao seu, só passa period/range pro fetcher)
  const [allBankSeriesMap, setAllBankSeriesMap] = useState<
    Record<number, SeriesData>
  >({});

  const handleBankSeriesLoaded = useCallback(
    (id: number, series: SeriesData) => {
      setAllBankSeriesMap((prev) => {
        if (prev[id] === series) return prev;
        return { ...prev, [id]: series };
      });
    },
    [],
  );

  const allBankSeriesList = useMemo(() => {
    if (!isAll) return [];
    return bankrolls.map((b) => ({
      id: b.id,
      name: b.name,
      series: allBankSeriesMap[b.id] ?? {
        dates: [],
        balances: [],
        netProfit: 0,
      },
    }));
  }, [isAll, bankrolls, allBankSeriesMap]);

  const colorPalette = ["#17ad1a", "#e42a2a", "#1976d2", "#9c27b0", "#ecec09"];

  const yScaleConfig = useMemo(() => {
    if (isAll || !bankroll) {
      return {
        minValue: undefined as number | undefined,
        maxValue: undefined as number | undefined,
      };
    }
    const initial = Number(bankroll.initialBalance ?? 0);
    const r = initial * 0.25;
    return { minValue: initial - r, maxValue: initial + r };
  }, [isAll, bankroll]);

  const xFormatter = useCallback(
    (date: Date) =>
      date.toLocaleString("pt-BR", {
        day: "2-digit",
        month: "2-digit",
        ...(period === "day" ? { hour: "2-digit", minute: "2-digit" } : {}),
      }),
    [period],
  );

  return (
    <Card elevation={0}>
      {isAll &&
        bankrolls.map((b) => (
          <BankrollSnapshotFetcher
            key={b.id}
            id={b.id}
            period={period}
            range={range}
            year={selectedDate.getFullYear()}
            resultFilter={resultFilter}
            onDataLoaded={handleBankSeriesLoaded}
          />
        ))}

      <CardContent>
        <Stack width="100%">
          <Stack
            direction="row"
            justifyContent="space-between"
            alignItems="center"
            sx={{ mb: 2 }}
          >
            <Typography variant="h6" component="span">
              {isAll ? "Todas as bancas" : bankroll?.name}
            </Typography>

            <Stack direction="row" spacing={2} alignItems="center">
              <FormControl size="small" sx={{ minWidth: 160 }}>
                <InputLabel>Período</InputLabel>
                <Select
                  label="Período"
                  value={period}
                  onChange={(e) => setPeriod(e.target.value as Period)}
                >
                  <MenuItem value="day">Dia</MenuItem>
                  <MenuItem value="week">Semana</MenuItem>
                  <MenuItem value="month">Mês</MenuItem>
                  <MenuItem value="year">Ano</MenuItem>
                </Select>
              </FormControl>

              <LocalizationProvider
                dateAdapter={AdapterDateFns}
                adapterLocale={ptBR}
              >
                <DatePicker
                  label="Data"
                  value={selectedDate}
                  onChange={(v) => v && setSelectedDate(v)}
                  slotProps={{
                    textField: {
                      size: "small",
                      sx: { minWidth: 170 },
                    } as Partial<React.ComponentProps<typeof TextField>>,
                  }}
                />
              </LocalizationProvider>

              <FormControl size="small" sx={{ minWidth: 180 }}>
                <InputLabel>Resultados</InputLabel>
                <Select
                  label="Resultados"
                  value={resultFilter}
                  onChange={(e) =>
                    setResultFilter(e.target.value as ResultFilter)
                  }
                >
                  <MenuItem value="all">Todos</MenuItem>
                  <MenuItem value="won">Somente Ganhos</MenuItem>
                  <MenuItem value="lost">Somente Perdas</MenuItem>
                  <MenuItem value="profitloss">Lucro/Prejuízo</MenuItem>
                </Select>
              </FormControl>
            </Stack>
          </Stack>

          {isLoadingContext || isLoadingSnapshots ? (
            <Typography sx={{ textAlign: "center", py: 4 }}>
              Carregando gráfico...
            </Typography>
          ) : error && !isAll ? (
            <Typography color="error" sx={{ textAlign: "center", py: 4 }}>
              Erro ao carregar dados do gráfico.
            </Typography>
          ) : (
            <>
              {isAll ? (
                <LineChart
                  xAxis={[
                    {
                      scaleType: "point",
                      data: allBankSeriesList[0]?.series.dates ?? [],
                      valueFormatter: xFormatter,
                    },
                  ]}
                  series={allBankSeriesList.map((b, i) => ({
                    label: b.name,
                    data: b.series.balances,
                    color: colorPalette[i % colorPalette.length],
                    showMark: true,
                  }))}
                  height={320}
                  grid={{ horizontal: true }}
                />
              ) : (
                <>
                  <LineChart
                    xAxis={[
                      {
                        scaleType: "point",
                        data: singleSeries.dates,
                        valueFormatter: xFormatter,
                      },
                    ]}
                    yAxis={[
                      {
                        min: yScaleConfig.minValue,
                        max: yScaleConfig.maxValue,
                      },
                    ]}
                    series={[
                      {
                        label: "Evolução",
                        data: singleSeries.balances,
                        color: profitColor,
                        showMark: true,
                        valueFormatter: (v) =>
                          v?.toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          }) ?? "",
                      },
                    ]}
                    height={320}
                    grid={{ horizontal: true }}
                  />

                  <Typography
                    variant="body2"
                    sx={{
                      alignSelf: "center",
                      mt: 1,
                      color: profitColor,
                      fontWeight: 600,
                    }}
                  >
                    {singleSeries.netProfit >= 0
                      ? "Lucro líquido: "
                      : "Prejuízo: "}
                    {singleSeries.netProfit.toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  </Typography>
                </>
              )}
            </>
          )}
        </Stack>
      </CardContent>
    </Card>
  );
}
