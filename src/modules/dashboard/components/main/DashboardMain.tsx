...
const DashboardMainPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    let totalWageredInUnits = 0;
    let totalResolvedWageredInUnits = 0;
    ...
    filteredEvents.forEach((event) => {
      ...
      const odd = Number(event.odd) || 0;

      // total investido geral (inclui pendentes)
      totalWageredInUnits += unitsStaked;
      ...
      if (event.result === "win") {
        const profitUnits = unitsStaked * (odd - 1);
        totalProfitUnits += profitUnits;
        totalResolvedWageredInUnits += unitsStaked;
        wins++;
        return;
      }

      if (event.result === "lose") {
        totalProfitUnits -= unitsStaked;
        totalResolvedWageredInUnits += unitsStaked;
        losses++;
        return;
      }

      if (event.result === "void" || event.result === "returned") {
        voids++;
      }
    });
    ...
    return {
      ...
      totalWageredInUnits: Number(totalWageredInUnits.toFixed(2)),
      totalResolvedWageredInUnits: Number(
        totalResolvedWageredInUnits.toFixed(2),
      ),
      winRate: decided > 0 ? Number(((wins / decided) * 100).toFixed(2)) : 0,
      roi:
        totalResolvedWageredInUnits > 0
          ? Number(
              ((totalProfitUnits / totalResolvedWageredInUnits) * 100).toFixed(
                2,
              ),
            )
          : 0,
        ...
        {!hasData ? (
          ...
          <Grid container spacing={3}>
            <Grid item xs={6} md={4} lg={2}>
              <MetricCard
                title="Lucro"
                value={`${metrics.totalProfitUnits} unids`}
                color={metrics.totalProfitUnits >= 0 ? "#17ad1a" : "#e42a2a"}
              />
            </Grid>
