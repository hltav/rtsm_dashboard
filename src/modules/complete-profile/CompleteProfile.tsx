...
//       // Se não tem user no contexto, não faz nada (o middleware redirecionará)
//       if (!user) return;

//       setIsLoadingInitialData(true);
//       try {
//         // O serviço agora não precisa passar IDs, o backend lê do cookie
...
//   // 🔹 Buscar dados do usuário logado (via sessão)
//   useEffect(() => {
//     const fetchUserProfileData = async () => {
//       if (!user) return;
...
//   const handleProfileImageUploadSuccess = useCallback(
//     (imageUrl: string) => {
//       if (!user) return; // 🔥 garante segurança
...
//       updateUser?.({
//         ...user, // agora TS sabe que user não é null
//         clientData: {
//           ...(user.clientData ?? {}),
//           image: imageUrl,
//         },
...
//   // 🔹 Submit do formulário (SESSION MODE)
//   const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
//     event.preventDefault();
...
//     const dataToSend = normalizeClientData(clientData);

//     try {
//       const { data } = await clientDataService.save(dataToSend);
