...
const FinishedEvents = () => {
  ...
  }, [finishedEvents, filters]);

  const uniqueEvents = Array.from(
    new Set(finishedEvents.map((e) => e.eventDescription)),
  );
  const uniqueSports = Array.from(new Set(finishedEvents.map((e) => e.sport)));
  const uniqueLeagues = Array.from(
    new Set(finishedEvents.map((e) => e.league)),
  );
  const uniqueMarkets = Array.from(
    new Set(finishedEvents.map((e) => e.market)),
  );
  const uniqueOdds = Array.from(new Set(finishedEvents.map((e) => e.odd)));
